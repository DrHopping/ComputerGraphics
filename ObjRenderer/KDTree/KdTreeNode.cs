...
namespace ObjRenderer.KDTree
{
    public class KdTreeNode
    {
        ...
        public Intersection? Traverse(in Ray ray)
        {
            ...
            if (intersectionResult.TMin >= intersectionResult.TMax)
            {
                return null;
            }
            ...
            var tSplit = (Split.SplitValue - AxisVal(ray.Origin, SplitAxis)) * AxisVal(invRayDir, SplitAxis);
        ...
        private Intersection? IntersectAllPrimitivesInLeaf(Ray ray)
        {
            Intersection? nearestHit = null;
            ...
            foreach (var triangle in Triangles)
            {
                ...
                if (intersection.HasValue && intersection.Value.T < nearestHit.Value.T)
                {
                    nearestHit = intersection.Value;
                }
            }
            return nearestHit;
        }
        ...
        private Triangle[][] SplitTriangles(BoxSplit split, int axis, int leftN, int rightN)
        {
            ...
            foreach (var triangle in Triangles)
            {
                ...
                if (tBoxMin <= splitPoint)
                {
                    result[0].Add(triangle);
                }

                if (tBoxMax >= splitPoint)
                {
